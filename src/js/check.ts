...
const request = (
  url: string,
  { follow = true, headOnly = false }: { follow?: boolean; headOnly?: boolean } = {},
): Promise<RequestResult> => new Promise((resolve, reject) => {
  let redirectCount = 0;
...
const decodeHtmlEntities = (title: string): string =>
  title
    .replace(/<[^>]+>/gu, '')
    .replace(/&amp;/gu, '&')
    .replace(/&lt;/gu, '<')
...
const run = async (): Promise<void> => {
  ...
  try {
    ...
    log.error(`Failed to connect to ${INVRT_URL}: ${(error as Error).message}`);
    process.exit(1);
    return;
  }
