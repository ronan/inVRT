...
export const loginAndSaveCookies = async (loginUrl, username, password, outputFile, options = {}) => {
    const { usernameSelector = 'input[name="username"],input[name="name"]', passwordSelector = '[type="password"]', waitForSelector = null, timeout = 30000, } = options;
    let browser;
    try {
...
const run = async () => {
    try {
        await loginAndSaveCookies(process.env.INVRT_LOGIN_URL || '', process.env.INVRT_USERNAME || '', process.env.INVRT_PASSWORD || '', process.env.INVRT_SESSION_FILE || '', {
