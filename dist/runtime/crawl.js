...
const resolveExcludeMatchers = (plan) => {
    const fromPlan = Array.isArray(plan.exclude)
        ? plan.exclude.filter((line) => typeof line === 'string' && line.trim() !== '')
        : [];
...
const isExcludedPath = (urlPath, rules) => rules.some((rule) => (rule.endsWith('*')
...
const crawl = async () => {
    ...
    const queue = seedPaths.map((entry) => ({ p: entry, depth: 0 }));
    ...
    try {
        while (queue.length > 0 && discovered.size < maxPages) {
            ...
            try {
                ...
                appendLog(`ERROR ${normalizedPath} ${error.message || String(error)}`);
                continue;
            }
...
const run = async () => {
    const { INVRT_URL, INVRT_CRAWL_DIR, INVRT_CRAWL_LOG, INVRT_MAX_CRAWL_DEPTH, INVRT_MAX_PAGES, INVRT_PROFILE, INVRT_ENVIRONMENT, } = process.env;
    if (!INVRT_URL) {
