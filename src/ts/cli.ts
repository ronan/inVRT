...
const packageJson = JSON.parse(fs.readFileSync(path.join(appRoot, 'package.json'), 'utf8')) as { version: string };
...
type SelectionCommandOptions = {
  name: string;
  description: string;
  bootMode?: 'boot' | 'boot-or-init';
  requiresConfig?: boolean;
  requiresLogin?: boolean;
  action: (context: BootedContext) => Promise<number | void>;
};

const countVerbosity = (argv: string[]): number =>
  argv.reduce((count, arg) => {
...
const stripVerbosity = (argv: string[]): string[] =>
  argv.filter((arg) => !/^--verbose$/u.test(arg) && !/^-v+$/u.test(arg));

const withSelectionOptions = <T extends Command>(command: T): T =>
  command
...
const resolveConfigFilepath = (env: Record<string, string>): string => {
...
const processInvrtEnv = (): Record<string, string> =>
  Object.fromEntries(
...
const boot = async (
  ...
  try {
    ...
    logger.debug(`Config read exception: ${(error as Error).message}`);
...
const canPromptForUrl = (): boolean => Boolean(process.stdin.isTTY);

const resolveInitUrl = async (config: Configuration, logger: Logger, candidate?: string): Promise<string> => {
...
const printConfig = (config: Record<string, string>): void => {
...
const printInfo = (info: Awaited<ReturnType<Runner['info']>>, logger: Logger): void => {
...
const registerSelectionCommand = (program: Command, logger: Logger, options: SelectionCommandOptions): void => {
...
const run = async (): Promise<void> => {
  ...
  withSelectionOptions(
    program.command('init')
      ...
      .option('--skip-baseline', 'Skip running baseline after init', false),
