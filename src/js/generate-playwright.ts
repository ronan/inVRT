...
import { extractPagesFromPlan, type HookMap } from './lib/plan-pages.js';
import { readStdin } from './lib/stdio.js';

const indentBlock = (content: string, spaces = 2): string =>
  content
    .split('\n')
    .map((line) => `${' '.repeat(spaces)}${line}`)
    .join('\n');

const resolveHookSource = (scriptValue: string, scriptsDir: string): string => {
...
const renderHookBlock = (scriptValue: string | undefined, scriptsDir: string): string => {
...
const run = async (): Promise<void> => {
  ...
  try {
    ...
    const titleCounts = scoped.reduce<Map<string, number>>((acc, entry) => {
